...
def interested_in(course, interests):
    """
    :param course: int course id
    :param interests: set of strings ex:{"Biology", "Teamwork"}
    :return: int number of shared interests
    """
...
def remaining_requirements(course_history, aal=0, ssc=0, sc=0, gp=0, us=0, misc=0):
    ...
    math2_taken = {413000, 420000, 427000, 473000, 253001, 281001, 256001, 282001, 307001, 316001, 317001, 320001,
                   345001, 347001, 348001, 351001, 352001, 391001, 392001, 394001, 395001, 397001, 411001, 412001,
                   413001, 414001, 415001, 421001, 422001, 431001, 432001, 433001, 434001, 441001, 444001, 445001,
                   446001, 461001, 462001, 463001, 467001} & course_history
    ...
    cs_electives_done = total_credits(cs_electives_taken_under | cs_electives_taken_over) >= 20 and total_credits(
        cs_electives_taken_over) >= 12
    ...
    science_taken = {201002, 202002, 203002, 251002, 252002, 253002, 221003, 222003, 223003, 141004, 321004, 322004,
                     323004, 201005, 202005, 203005, 201006, 348006, 301006, 304006, 305006, 211007, 111003, 113003,
                     221003, 212007, 213007} & course_history  # taken classes from science requirement
    ...
    # psy=[]
    # bi=[]
    science_done = {201002, 202002, 203002}.issubset(course_history) or {251002, 252002, 253002}.issubset(
        course_history) or \
                   ...
                   (211007 in course_history and len({111003, 113003, 221003} & course_history) >= 1 and len(
                       {212007, 213007} & course_history) >= 1)  # TODO
    ...
    if not cs_electives_done:
        ...
        if total_credits(cs_electives_taken_under) < 8:
            alttxt = " or (" + str(8 - total_credits(cs_electives_taken_under)) + " credits from " + str(
                id_to_title(cs_electives_under.difference(cs_electives_taken_under))) + " and " + str(
                12 - total_credits(cs_electives_taken_over)) + " credits from " + str(id_to_title(
                (cs_electives_over.difference(cs_electives_taken_over)).difference(skipped_shared_course))) + ")"
        remaining.add(
            "(" + str(20 - total_credits(cs_electives_taken_over | cs_electives_taken_under)) + " credits from " + str(
                id_to_title((cs_electives_under | cs_electives_over).difference(
                    cs_electives_taken_under | cs_electives_taken_over))) + ")" + alttxt)
    ...
    while self.has_remaining_requirements(course_hist, aal2, ssc2, sc2, gp2, us2, misc2, interests):  # runs each term
        ...
        for course in self.prioritize_requirements(
            ...
            if term_credits + get_credits(course) <= max_credits_per_term and (
                    ...
                    last_course_hist)) and is_offered(course, this_term, this_year):
                # if course == 210000:
...
def categorize_courses(course_set):
    ...
    for course in course_set:
        ...
        if cs_elec_left in course:  # In this case, we're checking for the CS electives (see line 98)
            parts = re.split(r'\) or \(|\) and \(',
                             course)  # ex: "10 credits from (CS0...) or (CS1...)" split into ["10 credits from ", "CS0...", " or ", "CS1..."]
            parts = [part.strip('()') for part in
                     parts]  # Remove () so now we have ex: ["10 credits from ", "CS0...", " or ", "CS1..."]
...
def arrange_electives(categorized_courses):
    ...
    if (len(categorized_courses['CS Elective Requirements']) > 0):
        ...
        credits_for_option_1 = credit_pattern.findall(categorized_courses["CS Elective Requirements"][
                                                          0])  # find all patterns matching the credit pattern expression
